    ...
    <div className="min-h-[100dvh] bg-[#f9fafb] flex flex-col relative overflow-hidden">
      ...
      <main className="relative z-10 flex-1 flex flex-col px-4 pt-10 pb-20">
        ...
        <div className="max-w-6xl mx-auto w-full flex flex-wrap items-center gap-x-8 gap-y-3 mb-12 animate-fade-in animate-delay-100">
          {[
            { icon: <Users size={14} strokeWidth={1.8} />, text: t("statsDocs", { count: totalAnalyses.toLocaleString() }) },
            { icon: <Clock size={14} strokeWidth={1.8} />, text: t("statsTime") },
            { icon: <Star size={14} strokeWidth={1.8} />, text: t("statsRating", { count: ratingCount.toLocaleString(), rating: averageRating }) },
          ].map((s) => (
            <div key={s.text} className="hidden sm:flex items-center gap-2.5 text-sm text-zinc-500 tabular-nums">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <aside className="lg:col-span-4 flex flex-col animate-slide-up animate-delay-300">
              ...
              <ul className="space-y-3">
                {[
                  { icon: <FileText size={16} strokeWidth={1.8} />, label: t("outcomeSummary") },
                  { icon: <ListChecks size={16} strokeWidth={1.8} />, label: t("outcomeClauses") },
                  { icon: <AlertTriangle size={16} strokeWidth={1.8} />, label: t("outcomeRisks") },
                ].map((item) => (
                  <li key={item.label} className="flex items-center gap-3 text-sm text-zinc-700">
                    <span className="text-emerald-600/90 shrink-0">{item.icon}</span>
                    {item.label}
                  </li>
                ))}
              </ul>
